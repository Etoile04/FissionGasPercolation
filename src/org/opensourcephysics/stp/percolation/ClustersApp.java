...
public class ClustersApp extends AbstractSimulation {
  ...
  PlotFrame plot3 = new PlotFrame("p", "Fraction of 'Open' GBs", "Fraction of 'Open' GBs");
  ...
  public void initialize() {
    ...
    meanClusterSize = new double[L*M];
    P_infinity = new double[L*M];
    P_span = new double[L*M];
    numClustersAccum = new double[L*M+1];
    numberOfTrials = 0;
    numberOfOpenGBs = 0;
  }
  ...
  public void doStep() {
    ...
    for(int i = 0;i<lattice.N;i++) {
      ...
      if((int) (pDisplay*lattice.N)==i) {//if i reaches number of occupied sites at pDisplay 
        for(int j = 0;j<lattice.N;j++) {
          numClustersAccum[j] += lattice.numClusters[j];
        }
        displayLattice();
        numberOfOpenGBs = lattice.getOpenGBs();
      }
    }
  ...
  private void plotAverages() {
    ...
    //for(int i = 0;i<lattice.N;i++) {
      ...
      plot3.append(0, p, numberOfOpenGBs/lattice.N);
    }
      for(int i = 0;i<lattice.N;i++) {
